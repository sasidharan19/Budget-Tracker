...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('pieChart', { static: false }) pieChartRef!: ElementRef<HTMLDivElement>;
  private svg: d3.Selection<SVGSVGElement, unknown, null, undefined> | null = null;

  ngOnInit() { this.load(); }
  ngAfterViewInit() { /* chart drawn after load */ }
  ...
  private drawPieChart() {
    ...
    // legend
    const legend = d3.select(container).append('div').style('margin-top','8px');
    legend.selectAll('div')
      ...
      .each(function(d:any,i){
        ...
        el.append('div').style('width','12px').style('height','12px').style('background', color(String(i)));
